...
export default function DistribusiPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const res = await distribusiApi.list({ page, limit });
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold">Distribusi Dana Zakat</h1>
            <p className="text-muted-foreground mt-1">Catat penyaluran dana zakat kepada Mustahiq</p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="py-8 text-center text-muted-foreground">Belum ada data Distribusi</div>
            ) : (
              <>
                    ...
                    <TableBody>
                      {list.map((d) => (
                        <TableRow key={d.id}>
                          ...
                          <TableCell>{d.Mustahiq?.nama || d.mustahiq?.nama || d.nama_mustahiq || '-'}</TableCell>
                          <TableCell>{d.NamaProgram?.nama || d.namaProgram?.nama || d.nama_program?.nama || '-'}</TableCell>
                          <TableCell className="text-right font-mono">Rp {(d.jumlah || 0).toLocaleString('id-ID')}</TableCell>
                          <TableCell>{d.Via?.nama || d.via?.nama || '-'}</TableCell>
